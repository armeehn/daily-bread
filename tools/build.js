...
function emitHeaders(){
  /* Cloudflare's Bot Fight Mode injects a per-request JavaScript Detections
     script into every HTML response it serves. The bytes therefore differ on
     every single request, which makes the last newsproof check -- "the bytes
     served right now still hash to the value that was signed" -- impossible to
     satisfy, and leaves every reader looking at the badge's red "could not be
     verified". On the Free plan that injection has no switch of its own: JS
     Detections is bound to Bot Fight Mode and cannot be disabled separately.

     Cloudflare's documented opt-out is an origin `Cache-Control: no-transform`,
     which suppresses the injection. We set it on exactly the pages that carry a
     published proof -- the sixteen FULL editions -- and nowhere else. Lite and
     e-ink carry no badge and no proof, and the assets are not hashed at all, so
     all of them keep edge compression.

     The cost, and it is a real one: `no-transform` also stops Cloudflare
     compressing these pages, so a full edition ships at its raw size (~113 kB)
     instead of brotli (~26 kB). That buys a verifiable edition. It does NOT
     affect the hash either way -- the badge reads `arrayBuffer()`, i.e. bytes
     that are already decompressed.

     Generated from LANGS so the header set can never drift from the set of
     editions that actually get signed. */
  const CACHE = 'public, max-age=0, must-revalidate, no-transform';
  const blocks = LANGS.map(L => `${url(L.code, 'full')}\n  Cache-Control: ${CACHE}`);
  const txt =
    '# GENERATED by tools/build.js -- do not edit by hand.\n' +
    '# Cloudflare Workers static-assets header rules. See emitHeaders() for why\n' +
    '# the full editions opt out of edge transforms (Bot Fight Mode / JS\n' +
    '# Detections injection breaks the newsproof live-bytes check).\n' +
    '# Only the FULL editions are listed: they are the ones with proofs.\n\n' +
    blocks.join('\n\n') + '\n';
  fs.writeFileSync(path.join(ROOT,'_headers'), txt);
  console.log('wrote _headers:', blocks.length, 'edition paths (no-transform)');
}
function clean(){
...
function main(){
  ...
  if(!onlyLang && !onlyVar) emitHeaders();
