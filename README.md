...
The site ships in **15 languages** (English plus the 12 the Government of BC
officially supports, plus Italian and Polish), matching the language set of the
sibling ripostelabs.xyz site. English lives at the root (`/`); every other
...
- `tools/assets/style.css` — the page stylesheet, inlined into each page.
- `tools/build.js` — renders every language from the strings. Run
  `node tools/build.js` (all languages) or `node tools/build.js fr` (one), or add
  `--clean` to remove the generated language dirs first. It also writes
  `sitemap.xml`. The generated pages are static (no runtime JS); editing shared
  content means editing `tools/strings/en.js` and re-running the build.
